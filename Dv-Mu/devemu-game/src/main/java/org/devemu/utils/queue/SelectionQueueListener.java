...
public class SelectionQueueListener extends QueueListener {
	AccountHelper accHelper;
	EventDispatcher dispatcher;
	MessageFactory factory;
	
	@Inject
	public SelectionQueueListener(AccountHelper aHelper,EventDispatcher dispatcher, MessageFactory factory) {
		accHelper = aHelper;
	...
	public boolean invoke(BaseClient o) {
		...
		if(o != null && o instanceof GameClient) {
			...
			remove(o);
			try {
				dispatcher.dispatch(new GameClientReuseEvent(client,factory.getMessage("AS", State.NULL)));
